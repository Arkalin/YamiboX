        ...
        .confirmationDialog(L10n.string("forum.native.upload"), isPresented: Binding(
            ...
            if let upload = pendingUpload {
                Button(L10n.string("forum.native.upload")) {
                    pendingUpload = nil
                    Task { await model.upload(file: upload.file, mimeType: upload.mimeType, configuration: upload.configuration, form: upload.form) }
                }
            }
        ...
        let list = List {
            ...
            ForEach(isEmbedded ? composerForm.map { [$0] } ?? [] : page.forms) { form in
                ForumFormSection(
                    ...
                    files: Binding(get: { model.selectedFiles[form.id] ?? [] }, set: { model.selectedFiles[form.id] = $0 }),
                    uploads: page.uploads.filter { form.kind == .blog ? $0.kind == .blogImage : form.kind == .thread && $0.kind != .blogImage },
                    attachments: model.attachments[form.id] ?? [],
                    disabled: formDisabled,
    ...
    private func importFile(_ result: Result<URL, Error>) {
        ...
        do {
            ...
            if let configuration = selection.configuration {
                ...
                var files = model.selectedFiles[selection.form.id] ?? []
                files.removeAll { $0.fieldName == field.name }
                files.append(.init(fieldName: field.name, file: file, mimeType: mimeType))
