...
enum ReaderInformationAnimation {
    ...
    static let titleCrossfadeDuration = 0.25
...
struct ReaderInformationTitleTransition: ViewModifier {
    let title: String
    @Environment(\.accessibilityReduceMotion) private var reduceMotion

    func body(content: Content) -> some View {
        // Each outgoing title keeps its own centered layout while fading.
        ZStack {
            content
        ...
        .animation(reduceMotion ? nil : .easeInOut(duration: ReaderInformationAnimation.titleCrossfadeDuration), value: title)
