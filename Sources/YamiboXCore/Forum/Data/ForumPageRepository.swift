...
public actor ForumPageRepository {
    ...
    public func fetchPage(url: URL, confirmedAction: Bool = false) async throws -> ForumPageDocument {
    ...
    public func submit(
    ...
    ) async throws -> ForumPageDocument {
        ...
        if (form.kind == .thread && { if case .thread = route { return true }; return false }()) ||
            ...
            return ForumPageDocument(url: response.url, title: page.title, message: L10n.string("forum.native.submitted"), continuationURL: response.url)
        }
        return page
    }
    ...
    private func parse(_ response: YamiboHTMLResponse) throws -> ForumPageDocument {
        ...
        return try LoadDiagnosticError.parsing(html: response.html, context: "ForumPageParser.parse") {
            try ForumPageParser.parse(html: response.html, url: response.url)
        }
    }
}
